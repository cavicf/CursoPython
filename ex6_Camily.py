...
class Cirurgia:
    ...
    def adicionaProf(self, prof):
       return self._equipe.append(prof)
    ...
    def equipeValida(self):
        ...
        for prof in self._equipe:
            ...
            elif isinstance(prof, Medico) and prof.especialidade == 'Anestesista':
                anestesista  = 1

            elif isinstance(prof, Instrumentador):
                instrumentador = 1
        if instrumentador == 1 and cirurgiao == 1 and anestesista == 1:
            return True
        else:
            return False      
    ...
    def calculaCustoCir(self):
        ...
        for prof in self._equipe:
            if isinstance(prof, Medico) and prof.especialidade == 'Cirurgião':
                custoTotal += self.tipoCirurgia._valorCirurgiao
            elif isinstance(prof, Medico) and prof.especialidade == 'Anestesista':
                custoTotal += self.tipoCirurgia._valorAnest
            elif isinstance(prof, Instrumentador):
                custoTotal += self.tipoCirurgia._valorInstrum
    
        if self._paciente.tipo == 'Convênio':
            return custoTotal - ((20/100) * custoTotal)
        elif self._paciente.tipo == 'Particular':
            return custoTotal
        else:
            return 0
